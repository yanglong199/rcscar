...
{
    public interface IReaderWriter        //描述读写性，
    {
        byte[] ReadBytes(DeviceAddress address, ushort size);            //DeviceAddress 是代表下位机地址
    ...
    public interface IDriver : IDisposable                                        //和连接有关系的接口
    {
        short ID { get; }
        string Name { get; }
